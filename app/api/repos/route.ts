...
export async function GET() {
  ...
  // Fetch which repos have the GitHub App installed
  ...
  if (installRes.ok) {
    ...
    await Promise.all(
      installations.map(async (inst: any) => {
        // "all repositories" installations cover everything without listing them
        ...
        if (repoRes.ok) {
          ...
          ;(repoData.repositories || []).forEach((r: any) => installedRepoNames.add(r.full_name))
        }
      })
    )
  }

  // Merge with Supabase settings
  ...
  const enriched = repos.map((repo: any) => ({
    ...
    app_installed: installedRepoNames.has(repo.full_name),  // new field
